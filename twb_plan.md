...
> **Дата:** Июль 2025 (актуализировано; доработка v3.0 — расширение режима тренера, кроссплатформенность, пропуски и статистика отклонений)
...
> Рекомендованный приоритет реализации: **(5) кроссплатформенность — заложить сразу**, далее (3,4) видимость/дни и (2) real-time тренера → (1) доступ по неделям → (7,8) пропуски и отклонения → (6) импорт по коду.
...
| **CP** | Кроссплатформенность | 🟡 сквозная | Поддержка iOS/Android/Desktop/Web с самого начала: адаптив, safe-area, темы, haptics с fallback, устойчивый WS (§12.1) | UI и сценарии работают на всех платформах Telegram + вебе |
