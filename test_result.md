...
backend:
  - task: "Phase1: Per-set logging (set_logs) — PATCH /sessions/{id}/exercise/{order}/set/{set_index}"
    ...
    status_history:
      - working: "NA"
        agent: "main"
        comment: "NEW PHASE 1 — по-подходное логирование факта. Test ONLY this new endpoint + related behavior (do NOT re-test unrelated phases). CHANGES: (1) SessionExercise += set_logs: List[SetLog] (SetLog={weight,reps,percent_1rm,done}) + rest_seconds (из плана, для таймера). set_logs строится при старте сессии разворотом sets_scheme (weight×sets×reps -> N индивидуальных подходов) ТОЛЬКО для основных упражнений (у подсобных is_accessory set_logs=[]). (2) NEW PATCH /api/sessions/{session_id}/exercise/{order}/set/{set_index} body {done?, weight?, reps?} query actor=athlete|coach & by=<telegram_id тренера>. Отмечает подход выполненным и/или пишет фактический вес/повторы. actor=coach требует by=тренер спортсмена (иначе 403; без by -> 400). После изменения: sets_scheme пересобирается ТОЛЬКО из done-подходов (_scheme_from_logs), tonnage пересчитывается; если ВСЕ подходы done -> exercise.status='done' + filled_by=actor + активируется следующий pending; если сняли отметку -> status='in_progress', снимается coach_confirmed; когда все упражнения done/skipped -> сессия авто-finished + замораживается stats; если сессия была finished, но подход сняли -> снова in_progress. Если правили вес/повторы -> exercise.edited=true. (3) Совместимость: PATCH /sessions/{id}/exercise/{order}?action=done теперь тоже помечает все set_logs done и пересобирает sets_scheme; action=reset снимает все set_logs done и возвращает плановую схему. (4) PATCH .../edit пересобирает set_logs под новую схему, сохраняя done по позициям. TEST PLAN: register athlete (email->Bearer), create plan from 'full-body-beginner' (или 'pl-autumn-3m' с maxes для весов), start session week=1 day=<workout>. (a) session.exercises[0].set_logs длиной == Σ target_sets первого упражнения; rest_seconds присутствует. (b) PATCH set/0 {done:true,weight:60,reps:5} -> ex.edited=true, sets_scheme=[{weight:60,sets:1,reps:5}], tonnage=300 (если reps=5,weight=60), status='in_progress'. (c) отметить все подходы done -> ex.status='done', filled_by='athlete', следующий ex стал 'in_progress'. (d) снять один подход (done:false) -> ex.status='in_progress', session.status!='finished'. (e) coach co-scribe: linked coach PATCH set/0 {done:true} actor=coach&by=<coach> -> 200, filled_by='coach'; unlinked coach -> 403; coach без by -> 400. (f) отметить все подходы всех упражнений -> session авто-finished, stats заморожены (sets_done, tonnage). (g) set_index вне диапазона -> 404; неизвестная сессия -> 404. Verify UUIDs/ISO/no _id leaks."
      - working: true
        agent: "testing"
...
test_plan:
  current_focus:
    - "Phase1: Per-set logging (set_logs) — PATCH /sessions/{id}/exercise/{order}/set/{set_index}"
  stuck_tasks: []
  test_all: false
